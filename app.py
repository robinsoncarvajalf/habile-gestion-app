...
st.set_page_config(page_title="Gestión Terapéutica & IA", page_icon="🧠", layout="wide")

# --- 1. SISTEMA DE USUARIOS Y AUTENTICACIÓN ---
# Base de datos simulada de usuarios (Usuario: Contraseña)
# Aquí puedes registrar a las personas que van a ocupar la app
...
# Inicializar estados de sesión para el login
...
# --- 2. BASE DE DATOS SIMULADA POR USUARIO (Mock Data) ---
# Separamos los pacientes y citas por cada usuario para que no se mezclen
if "datos_usuarios" not in st.session_state:
    st.session_state.datos_usuarios = {
        "katty": {
            ...
            "citas": [
                {"hora": "09:00", "paciente": "Facundo", "motivo": "Sesión Semanal Differential"},
...
if not st.session_state.autenticado:
    ...
    with col2:
        st.markdown("<h2 style='text-align: center;'>🔐 Iniciar Sesión</h2>", unsafe_url=True)
        st.write("Por favor, ingresa tus credenciales para acceder a tu agenda personalizada.")
        
        usuario_input = st.text_input("Usuario (Ej: katty, robin)").strip().lower()
    ...
    # --- 5. APLICACIÓN PRINCIPAL (SÓLO SI ESTÁ AUTENTICADO) ---
    usuario = st.session_state.usuario_actual
    
    # Obtener los datos específicos del usuario que inició sesión
    mis_pacientes = st.session_state.datos_usuarios[usuario]["pacientes"]
    mis_citas = st.session_state.datos_usuarios[usuario]["citas"]
    
    # Barra lateral de navegación y perfil
    ...
    if menu == "📅 Mi Agenda":
        ...
        if len(mis_citas) == 0:
            st.info("No tienes citas programadas para hoy, ¡un día tranquilo!")
        else:
        ...
        if len(mis_pacientes) == 0:
            ...
            with tab1:
                ...
                if st.button("Guardar Objetivo"):
                    st.session_state.datos_usuarios[usuario]["pacientes"][paciente_sel]["objective"] = nuevo_obj
                    st.success("¡Objetivo actualizado con éxito en tu cuenta!")
                    
            with tab2:
                st.subheader(f"Informe Psicológico - {paciente_sel}")
                st.text_input("Título del Informe", value="Informe de Avance Trimestral")
                desarrollo = st.text_area("Escribe observaciones...", height=150)
    ...
    # --- MÓDULO: ASISTENTE IA ---
        ...
        st.write("Planifica tus sesiones usando la IA basada en tus propios pacientes.")
